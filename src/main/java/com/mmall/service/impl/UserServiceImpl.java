    ...
    private ServerResponse<String> checkValid(String str, String type) {
        if (StringUtils.isNotBlank(type)) {
            if (Const.USERNAME.equals(type)) {
                if (userMapper.checkUsername(str) > 0) return ServerResponse.createByErrorMessage("用户已存在");
            }
            if (Const.EMAIL.equals(type)) {
                if (userMapper.checkEmail(str) > 0) return ServerResponse.createByErrorMessage("email已存在");
            }
        } else {
